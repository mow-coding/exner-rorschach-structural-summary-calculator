...
- The repository-root README in all five languages has two layers: one brief introduction and archive link for each earlier major version (currently v1 and v2), followed by a fuller summary of the **latest released version**. At every release, replace the latest-version summary, app link, current offer, and release-note/evidence links. When a new major version supersedes the current one, move the former current version to one brief archive entry and update the locale checker. Do not append older patch summaries or turn the root README into a release list; keep detailed history in `CHANGELOG*` and the version directories. Review all five README files together before publishing.
