...
A versão menor 1.4.0 foi aplicada.

> **Foi adicionado suporte multilíngue.**
>
- Cinco idiomas são suportados: coreano, inglês, japonês, espanhol e português.

    Um botão de idioma (🌐) no canto superior direito do aplicativo web permite trocar de idioma a qualquer momento.

- Todos os textos da interface, as mensagens dos modais, as orientações, os avisos de slots e o texto do rodapé foram traduzidos para os cinco idiomas.
- Todas as descrições de itens da aba Mais foram traduzidas para os cinco idiomas.
...
### *Suporte multilíngue adicionado*

O suporte a cinco idiomas (coreano, inglês, japonês, espanhol, português) foi adicionado ao aplicativo web,

e todo o texto da interface foi traduzido.

> **Botão de idioma**
>
- Botão com ícone de globo (🌐) adicionado no canto superior direito do aplicativo web
- Ao carregar a página, o JavaScript adiciona a classe `language-selector-collapsed` para mostrar o botão
- O botão de idioma aparece acima do modal inicial mesmo enquanto ele está aberto (z-index: 10001 > 1000)
- O botão de idioma continua visível depois que o modal inicial é fechado
- Ao passar o mouse, as opções de idioma (한국어, English, 日本語, Español, Português) são mostradas
- O idioma escolhido é salvo no armazenamento local e mantido na próxima visita

> **Detalhes técnicos**
>
- Estrutura dos dados de tradução

    ---

    - Objeto `translations`: traduções do texto básico da interface (201 entradas)

        ---

        - Elementos básicos do aplicativo (app_title, tab_scoring, tab_notice)
        - Texto dos botões (btn_calc, btn_reset, btn_add, btn_del)
        - Modal de primeiro acesso (modal_welcome_title, modal_welcome_new, modal_welcome_load, modal_welcome_provider etc.)
        - Modal de confirmação de redefinição (modal_reset_title, modal_reset_msg, modal_reset_yes, modal_reset_no)
        - Modal de solicitação de interpretação por IA (modal_ai_title, modal_ai_msg, modal_ai_open, modal_ai_cancel)
        - Links e outros (link_manual, link_bug, link_patch, link_lab, link_privacy)
        - Política de privacidade (privacy_title, privacy_notice_title, privacy_notice_text)
        - Mensagens de orientação (list_header, loader_calc, loader_sub, guide_response, tooltip_info)
        - Abas e botões da tela de resultados (tab_upper, tab_lower, tab_special, btn_rawdata, btn_summary, btn_print, btn_ai)
        - Rótulos das caixas de seleção de Special Indices (scon_main, scon_note, depi_main, cdi_main, hvi_main, obs_main, obs_r1, obs_r2, obs_r3)
        - Mensagens de aviso de slots (toast_warning_over_scoring, toast_warning_category_dup, toast_warning_hierarchy, toast_warning_too_many_responses, toast_warning_validity, toast_info_reset e o texto de cada mensagem)
        - Texto do rodapé (footer_title, footer_copyright)

    - Objeto `listTranslations`: traduções das descrições de itens da aba Mais

        ---

        - `category`: nomes de categorias (10 categorias)
        - `categoryDesc`: descrições de categorias (10 categorias)
        - `items`: descrições de itens individuais (106 itens)
...
- Atualização dinâmica do texto
    ...
    - Função `changeLanguage()`: atualiza dinamicamente todo o texto da interface ao trocar de idioma
    - Função `updateAllTexts()`: atualiza todos os elementos de texto conforme o idioma selecionado
    - Função `generateListTable()`: regenera a tabela da aba Mais no idioma atual
    - Função `updateLanguageButtons()`: atualiza o estado ativo dos botões de idioma

- Funções de tradução

    ---

    - `t(key)`: recebe uma chave de tradução e retorna o texto do idioma atual
    - `getTranslatedCategoryName()`: obtém a tradução do nome da categoria
    - `getTranslatedCategoryDescription()`: obtém a tradução da descrição da categoria
    - `getTranslatedItemDescription()`: obtém a tradução da descrição do item


### *Situação da tradução e comparação por idioma*

Todos os elementos da interface e as descrições da aba Mais estão totalmente traduzidos para os cinco idiomas.
...
> | --- | --- | --- |
> | **Texto básico da interface** | 45 | 45 |
> | **Abas/botões de resultados** | 7 | 7 |
> | **Rótulos de caixas de seleção** | 9 | 9 |
> | **Mensagens de aviso de slots** | 12 | 12 |
> | **Texto do rodapé** | 2 | 2 |
> | **Nomes de categorias** | 10 | 10 |
> | **Descrições de categorias** | 10 | 10 |
> | **Descrições de itens** | 106 | 106 |
> | **Total** | **201** | **201** |
>
...
> >
> >
> > | Idioma | Concluído | Completude |
> > | --- | --- | --- |
> > | **Coreano (ko)** | 201 | 100% ✅ |
> > | **Inglês (en)** | 201 | 100% ✅ |
> > | **Japonês (ja)** | 201 | 100% ✅ |
> > | **Espanhol (es)** | 201 | 100% ✅ |
> > | **Português (pt)** | 201 | 100% ✅ |
> >
> > ✅ **A tradução está completa em todos os idiomas.**
> >

### *Comparação das traduções dos principais elementos da interface por idioma*

Se alguma tradução estiver insuficiente, avise; ela será corrigida.

> **Elementos básicos do aplicativo**
>

| Chave | Coreano (ko) | Inglês (en) | Japonês (ja) | Espanhol (es) | Português (pt) |
| --- | --- | --- | --- | --- | --- |
...
> **Modal de solicitação de interpretação por IA**
>
...
> **Abas e botões da tela de resultados**
>
...
> **Rótulos das caixas de seleção de Special Indices**
>
...
> **Nomes de categorias da aba Mais**
>

| Chave de categoria | Coreano (ko) | Inglês (en) | Japonês (ja) | Espanhol (es) | Português (pt) |
| --- | --- | --- | --- | --- | --- |
...
> **Descrições de categorias e de itens da aba Mais**

Todas as descrições de categorias (10) e de itens (106) da aba Mais estão traduzidas para os cinco idiomas.
>
- **Descrições de categorias (10)**

    ---

    | Categoria | Coreano (ko) | Inglês (en) | Japonês (ja) | Espanhol (es) | Português (pt) |
    | --- | --- | --- | --- | --- | --- |
...
- **Card (10 itens)**
    ...
    | Código do item | Coreano (ko) | Inglês (en) | Japonês (ja) | Espanhol (es) | Português (pt) |
    | --- | --- | --- | --- | --- | --- |
...
> **Atributos title**
>
- **Botões de idioma**: atributos `title` do botão com ícone de globo e de cada botão de idioma
- **Botões de exportação CSV**: atributos `title` dos botões "Raw Data" e "Summary"
- **Botões de controle de slots**: atributos `title` dos botões "+" e "-" da área de pontuação

| Chave | Coreano (ko) | Inglês (en) | Japonês (ja) | Espanhol (es) | Português (pt) |
| --- | --- | --- | --- | --- | --- |
| `title_select_language` | 언어 선택 | Select Language | 言語を選択 | Seleccionar Idioma | Selecionar Idioma |
- `title_lang_ko`, `title_lang_en`, `title_lang_ja`, `title_lang_es`, `title_lang_pt`: title de cada botão de idioma
- `title_export_rawdata`: title do botão de exportação CSV (Raw Data)
- `title_export_summary`: title do botão de exportação CSV (Summary)
- `title_add_slot`: title do botão de adicionar slot
- `title_delete_slot`: title do botão de excluir slot

### *Melhoria do estilo da aba Mais*

Para resolver o transbordamento da tabela por textos longos na aba Mais, foram feitas as seguintes mudanças

- **Sem quebras de linha**: `word-wrap`, `word-break` e `overflow-wrap` removidos; `white-space: nowrap` aplicado
- **Largura do cabeçalho**: `table-layout: fixed` removido; `min-width: 800px` definido na tabela
- **Rolagem horizontal**: `overflow-x: auto` adicionado a `.table-wrapper:has(#list-table-body)`
- **Células de descrição**: `white-space: normal` mantido para que descrições longas possam quebrar linha

### *Melhoria da qualidade do código*

Comentários inline desnecessários foram removidos e a consistência do sumário e dos comentários foi melhorada.

- Comentários inline organizados
- Consistência do sumário e dos comentários

---

# Roteiro futuro

A inclusão de anúncios em banner está sendo considerada para a próxima versão principal.
