...
Minor release 1.4.0 has been applied.

> **Multilingual support has been added.**
>
- Five languages are supported: Korean, English, Japanese, Spanish, and Portuguese.

    A language button (🌐) at the top right of the web app lets you switch languages at any time.

- All UI text, modal messages, guidance, slot warnings, and footer text have been translated into the five languages.
- Every item description on the More tab has been translated into the five languages.
...
Support for five languages (Korean, English, Japanese, Spanish, Portuguese) was added to the web app,

and all UI text was translated.

> **Language button**
>
- Globe icon (🌐) button added at the top right of the web app
- On page load, JavaScript adds the `language-selector-collapsed` class to show the button
- The language button shows above the initial modal even while it is open (z-index: 10001 > 1000)
- The language button stays visible after the initial modal closes
- Hovering shows the language options (한국어, English, 日本語, Español, Português)
- The chosen language is saved in local storage and kept on the next visit
...
- Translation data structure
    ...
    - `translations` object: basic UI text translations (201 entries)

        ---

        - Basic application elements (app_title, tab_scoring, tab_notice)
        - Button text (btn_calc, btn_reset, btn_add, btn_del)
        - First-visit modal (modal_welcome_title, modal_welcome_new, modal_welcome_load, modal_welcome_provider, etc.)
        - Reset confirmation modal (modal_reset_title, modal_reset_msg, modal_reset_yes, modal_reset_no)
        - AI interpretation request modal (modal_ai_title, modal_ai_msg, modal_ai_open, modal_ai_cancel)
        - Links and others (link_manual, link_bug, link_patch, link_lab, link_privacy)
        - Privacy policy (privacy_title, privacy_notice_title, privacy_notice_text)
        - Guidance messages (list_header, loader_calc, loader_sub, guide_response, tooltip_info)
        - Results screen tabs and buttons (tab_upper, tab_lower, tab_special, btn_rawdata, btn_summary, btn_print, btn_ai)
        - Special Indices checkbox labels (scon_main, scon_note, depi_main, cdi_main, hvi_main, obs_main, obs_r1, obs_r2, obs_r3)
        - Slot warning messages (toast_warning_over_scoring, toast_warning_category_dup, toast_warning_hierarchy, toast_warning_too_many_responses, toast_warning_validity, toast_info_reset and each message text)
        - Footer text (footer_title, footer_copyright)

    - `listTranslations` object: More-tab item description translations

        ---

        - `category`: category names (10 categories)
        - `categoryDesc`: category descriptions (10 categories)
        - `items`: individual item descriptions (106 items)
...
- Dynamic text updates
    ...
    - `changeLanguage()`: dynamically updates all UI text when the language changes
    - `updateAllTexts()`: updates every text element to the currently selected language
    - `generateListTable()`: regenerates the More-tab table in the current language
    - `updateLanguageButtons()`: updates the active state of the language buttons

- Translation functions

    ---

    - `t(key)`: takes a translation key and returns the text for the current language
    - `getTranslatedCategoryName()`: gets the translated category name
    - `getTranslatedCategoryDescription()`: gets the translated category description
    - `getTranslatedItemDescription()`: gets the translated item description


### *Translation status and comparison by language*

All UI elements and More-tab item descriptions are fully translated into the five languages.
...
> | --- | --- | --- |
> | **Basic UI text** | 45 | 45 |
> | **Results screen tabs/buttons** | 7 | 7 |
> | **Checkbox labels** | 9 | 9 |
> | **Slot warning messages** | 12 | 12 |
> | **Footer text** | 2 | 2 |
> | **Category names** | 10 | 10 |
> | **Category descriptions** | 10 | 10 |
> | **Item descriptions** | 106 | 106 |
> | **Total** | **201** | **201** |
>
...
> >
> >
> > | Language | Done | Completeness |
> > | --- | --- | --- |
> > | **Korean (ko)** | 201 | 100% ✅ |
> > | **English (en)** | 201 | 100% ✅ |
> > | **Japanese (ja)** | 201 | 100% ✅ |
> > | **Spanish (es)** | 201 | 100% ✅ |
> > | **Portuguese (pt)** | 201 | 100% ✅ |
> >
> > ✅ **Translation is complete for every language.**
> >

### *Translation comparison of the main UI elements by language*

Please report any translation that falls short; it will be corrected.

> **Basic application elements**
>

| Key | Korean (ko) | English (en) | Japanese (ja) | Spanish (es) | Portuguese (pt) |
| --- | --- | --- | --- | --- | --- |
...
> **AI interpretation request modal**
>
...
> **Results screen tabs and buttons**
>
...
> **Special Indices checkbox labels**
>
...
> **More-tab category names**
>

| Category key | Korean (ko) | English (en) | Japanese (ja) | Spanish (es) | Portuguese (pt) |
| --- | --- | --- | --- | --- | --- |
...
> **More-tab category descriptions and item descriptions**

All category descriptions (10) and item descriptions (106) on the More tab are translated into the five languages.
>
- **Category descriptions (10)**

    ---

    | Category | Korean (ko) | English (en) | Japanese (ja) | Spanish (es) | Portuguese (pt) |
    | --- | --- | --- | --- | --- | --- |
...
- **Card (10 items)**
    ...
    | Item code | Korean (ko) | English (en) | Japanese (ja) | Spanish (es) | Portuguese (pt) |
    | --- | --- | --- | --- | --- | --- |
...
> **Title attributes**
>
- **Language buttons**: `title` attributes of the globe icon button and each language button
- **CSV export buttons**: `title` attributes of the "Raw Data" and "Summary" buttons
- **Slot control buttons**: `title` attributes of the "+" and "-" buttons in the scoring area

| Key | Korean (ko) | English (en) | Japanese (ja) | Spanish (es) | Portuguese (pt) |
| --- | --- | --- | --- | --- | --- |
| `title_select_language` | 언어 선택 | Select Language | 言語を選択 | Seleccionar Idioma | Selecionar Idioma |
- `title_lang_ko`, `title_lang_en`, `title_lang_ja`, `title_lang_es`, `title_lang_pt`: title of each language button
- `title_export_rawdata`: title of the CSV export (Raw Data) button
- `title_export_summary`: title of the CSV export (Summary) button
- `title_add_slot`: title of the add-slot button
- `title_delete_slot`: title of the delete-slot button

### *More-tab style improvement*

To stop long text on the More tab from overflowing the table, the following changes were made

- **No line wrapping**: removed `word-wrap`, `word-break`, and `overflow-wrap`; applied `white-space: nowrap`
- **Header width**: removed `table-layout: fixed`; set `min-width: 800px` on the table
- **Horizontal scrolling**: added `overflow-x: auto` to `.table-wrapper:has(#list-table-body)`
- **Description cells**: kept `white-space: normal` so long descriptions can wrap

### *Code quality*

Unnecessary inline comments were removed, and the consistency of the table of contents and comments was improved.

- Inline comments tidied
- Table of contents and comment consistency

---

# Roadmap

Adding banner ads is being considered for the next major release.
